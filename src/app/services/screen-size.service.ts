...
export class ScreenSizeService {
  ...
  get isSmall(): boolean {
    return this.state().breakpoints[this.SMALL_SCREEN] ?? false;
  }

  get isMedium(): boolean {
    return this.state().breakpoints[this.MEDIUM_SCREEN] ?? false;
  }

  get isLarge(): boolean {
    return this.state().breakpoints[this.LARGE_SCREEN] ?? false;
  }
}
